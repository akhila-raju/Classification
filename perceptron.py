...
class PerceptronClassifier(object):
    ...
    def train(self, input_train_data, label_train_data, input_val_data, label_val_data, iterations, callback=None):
        ...
        # for each training point (x[i], y[i]*). x[i] is the point and y[i]* is what it should be labeled.
        # y = classify(x[i]). classify = x[i] * w[y]. so you test every weight given and find the largest
        # value. if this value = y[i]*, then do nothing. otherwise, you have to adjust the weights.
        ...
        for iteration in range(iterations):
            ...
            for i in range(len(input_train_data)):
                ...
                y = self.classify(f)
                trueLabel = label_train_data[i]

                if y != trueLabel:
                    self.weights[trueLabel] += f
                    self.weights[y] -= f
    ...
    def find_high_weight_features(self, label):
        ...
        "*** YOUR CODE HERE ***"
        feature_vects = self.weights[label]
        feature_vects = feature_vects.sortedKeys()
        for i in range(100):
            best100Features.append(feature_vects[i])
