...
def enhancedFeatureExtractor(datum):
    ...
    We keep track a features array for out ternary represenation. We run dfs to keep expanding the white pixels
    along the N*N matrix.  
    ##
    """
    ...
    for i in range(DIGIT_DATUM_HEIGHT):
        for j in range(DIGIT_DATUM_WIDTH):
            if datum[i][j] == 0:
                if (i, j) not in generalArray:
                    counter += 1
                    generalArray+=dfs(datum, (i, j), generalArray)
    myFeatures = [0, 0, 0]
    if counter == 1:
        myFeatures[0] = 1
    elif counter == 2:
...
def dfs(datum, state, generalArray):
    stack = util.Stack()
    stack.push(state)
    checked = []
    while not stack.isEmpty():
        curr = stack.pop()
        if curr not in checked and curr not in generalArray:
            checked.append(curr)
