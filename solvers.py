...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        for i in range(len(param_vars)):
            new_param_tensor = param_vars[i] - alpha * grad_tensors[i]
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        for i in range(len(param_vars)):
            new_value_tensor = momentum * vel_vars[i] - alpha * grad_tensors[i]
            updates.append( (vel_vars[i], new_value_tensor) )
            new_param_tensor = param_vars[i] + new_value_tensor
            updates.append( (param_vars[i], new_param_tensor) )
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        Args:
            input_train_data: a numpy.array with shape (N, R)
                (N = # training samples. R = # variables)
        ...
        # this is what we want the model to end up looking like
        ...
        "*** YOUR CODE HERE ***"
        # model.get_param_vars is theta.
        # tensor flow is like a lambda, but you can differentiate it.
        loss_tensor = self.get_loss_tensor(model.prediction_tensor, target_ph, model.get_param_vars(regularizable=True))
        # updates computes gradients
        updates = self.get_updates(loss_tensor, model.get_param_vars(trainable=True))
        # update_ops tells you what you do with the gradient
        update_ops = [tf.assign(old_var, new_var_or_tensor) for (old_var, new_var_or_tensor) in updates]

        train_losses = []
        val_losses = []
        train_dict = {placeholders[0]:train_data[0], placeholders[1]:train_data[1]}
        val_dict = {placeholders[0]: val_data[0], placeholders[1]: val_data[1]}

        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"

            train_loss = session.run(loss_tensor, train_dict)

            session.run(update_ops, feed_dict=train_dict)

            val_loss = session.run(loss_tensor, val_dict)
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        #instatiate generators for validation and training data
        ...
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
            # use generator mentioned in project hints

            next_train = train_gen.next()
            next_val = val_gen.next()

            train_dict = {placeholders[0]: next_train[0], placeholders[1]: next_train[1]}
            val_dict = {placeholders[0]: next_val[0], placeholders[1]: next_val[1]}
