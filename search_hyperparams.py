...
def search_hyperparams(train_data, train_labels, val_data, val_labels,
    ...
    for model, (learning_rate, momentum, batch_size) in zip(models, hyperparams):
        "*** YOUR CODE HERE ***"
        #want loss to be lower

        solver_ = solvers.MinibatchStochasticGradientDescentSolver(learning_rate, iterations, batch_size, momentum)
        loss_tuple = solver_.solve(train_data,train_labels,val_data,val_labels, model)
        last_loss = loss_tuple[1][-1]
        if last_loss < temp_model:
            best_model = model
            temp_model = last_loss
